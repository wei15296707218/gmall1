...
//        orderTotalPrice=getOrderPrice(orderItemList);
        ServerResponse<Order> order_serveresponse = createOrder(userId,shippingId,orderItemList);
//
//        int a=3/0;
//
        if(!order_serveresponse.isSuccess()){
            return order_serveresponse;
        }
        Order order = order_serveresponse.getData();
//        //step5:将List<OrderItem>保存到数据库
       ...
       //step7:购物车中清空已下单的商品
       ServerResponse serverResponse1 = cartService.cleanCart(cartlist);
       if (!serverResponse1.isSuccess()){
           return ServerResponse.createServerResponseByError(ResponseCode.ERROR,"购物车为空或清空购物车失败");
       }
...
//
 //  @Override
   // public ServerResponse get_order_cart_product(Integer userId) {
     //step1:查询购物车
     //  ServerResponse<List<Cart>>  cartList= (ServerResponse<List<Cart>>) cartMapper.findCartsByUseridAndChecked(userId);
    ...
    private  ServerResponse  reduceProductStock(List<OrderItem> orderItemList){
        ...
        if(orderItemList!=null&&orderItemList.size()>0){
            ...
            for(OrderItem orderItem:orderItemList){
                ...
                ServerResponse serverResponse1 = productService.reduceProductStock(productId,count);
               if (!serverResponse1.isSuccess()){
...
//
    ...
    private ServerResponse createOrder(Integer userId, Integer shippingId,List<OrderItem> orderItemList){
        ...
        order.setPostage(0);
        order.setPaymentType(PaymentEnum.PAYMENT_ONLINE.getCode());
...
//
    @Override
    public String alipay_callback(Map<String, String> requestParams) {
        ...
        payInfo.setPayPlatform(PaymentEnum.PAYMENT_ONLINE.getCode());
...
private static Log log = LogFactory.getLog(Main.class);
    ...
    public ServerResponse pay(Order order) {
       ...
       List<OrderItem> orderItemList =  orderItemMapper.findOrderItemByOrderNo(order.getOrderNo());
        if (orderItemList==null||orderItemList.size()<=0){
            return ServerResponse.createServerResponseByError(ResponseCode.ERROR,"没有可以购买的商品");
        }
        // 商品明细列表，需填写购买商品详细信息，
