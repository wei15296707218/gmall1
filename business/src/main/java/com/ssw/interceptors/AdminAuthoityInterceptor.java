 ...
 */
@Component
public class AdminAuthoityInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws UnsupportedEncodingException {
       ...
       HttpSession session =  request.getSession();
        ...
        if (role== RoleEnum.ROLE_USER.getRole()){
            ...
            try {
                PrintWriter printWriter = response.getWriter();
                ServerResponse serverResponse =  ServerResponse.createServerResponseByError(ResponseCode.NOT_LOGIN,"权限不足");
                Gson gson = new Gson();
                String json = gson.toJson(serverResponse);
                printWriter.write(json);
                printWriter.flush();
